...
void fir_filter_setup_hw( struct wav* ir, uint16_t channel )
{
	...
	switch ( channel )
	{
		case 0: // left channel
			...
			{
				// Set Coefficients
				IOWR( FIR_0_BASE, i, wav_get_uint16( ir, 2*i ) );
				
				// init Input FIFOs 
				IOWR_ALTERA_AVALON_FIFO_DATA( M2S_FIFO0_BASE, 0x00000000 );
			}
			
			for ( i = 0; i< 512; i++ )
				// init Output FIFOs 
				(void)IORD_ALTERA_AVALON_FIFO_DATA( S2M_FIFO0_BASE );
			}		
			break;
		...
		case 1: // right channel
			...
			{
				// Set Coefficients
				IOWR( FIR_0_BASE, i, wav_get_uint16( ir, 2*i+1 ) );
...
)
{
    ...
    for ( kk = 0; kk < 512; kk++ )
    {
		...
		IOWR_ALTERA_AVALON_FIFO_DATA( M2S_FIFO0_BASE, ii );
		
		// Read result from FIFO
		temp_32 (int32_t)IORD_ALTERA_AVALON_FIFO_DATA( S2M_FIFO0_BASE );
