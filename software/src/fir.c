...
void fir_filter_setup_hw( struct wav* ir, uint16_t channel )
{
	...
	printf( "fir_filter_setup_hw()\n" );
			
	switch ( channel )
	{
		case 0: // left channel
			printf( "Set Coefficients and init Input FIFOs for left channel\n" );
			for ( i = 0; i< 512; i++ )
			{
				// Set Coefficients
				...
				// init Input FIFOs 
				IOWR( M2S_FIFO_FIR_L_BASE, 0, 0x00000000 );
				//IOWR_ALTERA_AVALON_FIFO_DATA( M2S_FIFO_FIR_L_BASE, 0x00000000 );
			}
			
			printf( "init Output FIFOs for left channel\n" );
			for ( i = 0; i< 512; i++ )
			{
				// init Output FIFOs 
		...
		case 1: // right channel
			printf( "Set Coefficients and init Input FIFOs for right channel\n" );
			for ( i = 0; i< 512; i++ )
			{
				// Set Coefficients
			...
			printf( "init Output FIFOs for right channel\n" );
			for ( i = 0; i< 512; i++ )
			{
				// init Output FIFOs 
